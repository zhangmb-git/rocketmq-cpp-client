...
int CSslSocket::Send(void *buf, int len) {
    ...
    if (n > 0) {
        if (m_io_state == SSL_IO_WANT_READ) {
            m_io_state = SSL_IO_NORMAL;
            log("Send: WANT_READ fulfilled");
        } else if (m_io_state == SSL_IO_WANT_READ_STACKED) {
            m_io_state = SSL_IO_NORMAL;
            log("Send: WANT_READ fulfilled, call stacked read handler");
    ...
    if (sslerr == SSL_ERROR_WANT_READ) {
        loge("Send: peer started SSL renegotiation");
        if (m_read_active) {
...
int CSslSocket::Recv(void *buf, int len) {
    ...
    if (m_renegotiation) {
        loge("SSL renegotiation disabled!!!");
        return NETLIB_ERROR;
    }
    ...
    if (n > 0) {
        if (m_io_state == SSL_IO_WANT_WRITE) {
            m_io_state = SSL_IO_NORMAL;
            log("Recv: WANT_WRITE fulfilled");
        } else if (m_io_state == SSL_IO_WANT_WRITE_STACKED) {
            m_io_state = SSL_IO_NORMAL;
            log("Recv: WANT_WRITE fulfilled, call stacked write handler");
...
void CSslSocket::handshakeTimeout(void *data, uint8_t msg, uint32_t handle, void *pParam) {
    ...
    if (data != NULL) {
        ...
        if (pSocket->m_state == SOCKET_STATE_WAIT_HANDSHAKE
            ...
            loge("SSL Handshake timeout, fd:%d, peer addr(%s:%d)",
                 pSocket->m_socket, pSocket->m_remote_ip.c_str(), (int) pSocket->m_remote_port);
...
int CSslSocket::doHandshake() {
    ...
    if (sslerr == SSL_ERROR_WANT_READ) {
        ...
        loge("SSL_ERROR_SYSCALL:%s", strerror(errno));
    } else {
