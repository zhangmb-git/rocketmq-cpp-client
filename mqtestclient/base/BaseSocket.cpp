...
int CBaseSocket::Listen(const char *server_ip, uint16_t port, callback_t callback, void *callback_data) {
    ...
    if (m_socket == INVALID_SOCKET) {
        ERROR("socket failed, err_code=%d\n", _GetErrorCode());
        return NETLIB_ERROR;
    }
    ...
    if (ret == SOCKET_ERROR) {
        ERROR("bind failed, err_code=%d ,  err=%s", _GetErrorCode(), strerror(errno));
    ...
    if (ret == SOCKET_ERROR) {
        ERROR("listen failed, err_code=%d", _GetErrorCode());
        closesocket(m_socket);
        return NETLIB_ERROR;
    }
...
net_handle_t CBaseSocket::Connect(const char *server_ip, uint16_t port, callback_t callback, void *callback_data) {
    ...
    m_socket = socket(AF_INET, SOCK_STREAM, 0);
    if (m_socket == INVALID_SOCKET) {
        ERROR("socket failed, err_code=%d", _GetErrorCode());
    ...
    if ((ret == SOCKET_ERROR) && (!_IsBlock(_GetErrorCode()))) {
        ERROR("connect failed, err_code=%d", _GetErrorCode());
        closesocket(m_socket);
        return NETLIB_INVALID_HANDLE;
    }
...
int CBaseSocket::Send(void *buf, int len) {
    ...
    if (ret == SOCKET_ERROR) {
        ...
        if (_IsBlock(err_code)) {
            ...
            WARN("!!!send failed, error code: %d", err_code);
        }
    }
...
void CBaseSocket::OnRead() {
    if (m_state == SOCKET_STATE_LISTENING) {
        ...
        if ((ioctlsocket(m_socket, FIONREAD, &avail) == SOCKET_ERROR) || (avail == 0)) {
            DEBUG("base socket ioctlsocket failed, err_code=%d, %s", _GetErrorCode(), strerror(errno));
...
#endif
        if (error) {
            WARN("getsockopt failed, err_code=%d, %s", _GetErrorCode(), strerror(errno));
            m_callback(m_callback_data, NETLIB_MSG_CLOSE, (net_handle_t) m_socket, NULL);
        } else {
...
void CBaseSocket::SetSendBufSize(int send_size) {
    ...
    if (rc == false) {
        ERROR("set SO_SNDBUF failed for fd=%d", m_socket);
    }
}
...
void CBaseSocket::SetRecvBufSize(int recv_size) {
    ...
    if (rc == false) {
        ERROR("set SO_RCVBUF failed for fd=%d", m_socket);
    }
}
...
#endif
    if (ret == SOCKET_ERROR) {
        ERROR("setNonblock failed, err_code=%d", _GetErrorCode());
    }
}
...
void CBaseSocket::setReuseAddr(bool on) {
    ...
    if (false == setIntOption(SO_REUSEADDR, reuse)) {
        ERROR("_setReuseAddr failed, err_code=%d", _GetErrorCode());
    }
}
...
void CBaseSocket::setTcpNoDelay(bool noDelay) {
    ...
    if (ret == SOCKET_ERROR) {
        ERROR("_setTcpNoDelay failed, err_code=%d", _GetErrorCode());
    }
}
...
void CBaseSocket::_SetAddr(const char *ip, const uint16_t port, sockaddr_in *pAddr) {
    ...
    if (pAddr->sin_addr.s_addr == INADDR_NONE) {
        ...
        if (host == NULL) {
            ERROR("gethostbyname failed, ip=%s", ip);
            return;
        }
