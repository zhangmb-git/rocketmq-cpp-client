...
//    curl_easy_setopt(curl, CURLOPT_POST, 1);
    ...
    if (CURLE_OK != res) {
        ERROR("curl_easy_perform failed, res=%d", res);
        return "";
    }
    ...
    if (!reader.parse(strResp, value)) {
        ERROR("json parse failed: %s", strResp.c_str());
        return "";
    }

    if (value["error_code"].isNull()) {
        ERROR("no code in response %s", strResp.c_str());
        return "";
    }
    uint32_t nRet = value["error_code"].asUInt();
    if (nRet != 0) {
        ERROR("upload faile:%u", nRet);
        return "";
    }
...
bool CHttpClient::DownloadByteFile(const string &url, AudioMsgInfo *pAudioMsg) {
    ...
    if (CURLE_OK != res || retcode != 200) {
        ERROR("curl_easy_perform failed, res=%d, ret=%u", res, retcode);
    }
    double nLen = 0;
