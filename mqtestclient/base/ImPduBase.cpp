...
void CImPdu::SetServiceId(uint16_t service_id) {
    ...
    m_pdu_header.service_id = service_id;
}
...
void CImPdu::SetPBMsg(const google::protobuf::MessageLite *msg) {
    ...
    if (!msg->SerializeToArray(szData, msg_size)) {
        WARN("pb msg miss required fields.");
    }
    ...
    // 一次分配足够的内存空间
    // modify_by yingchun.xu 2019-07-17
    //m_buf.Write(NULL, sizeof(PduHeader_t));
    m_buf.Write(NULL, sizeof(PduHeader_t) + msg_size);
    m_buf.Read(NULL, msg_size);
